...
#pragma unroll
        for (int i = 0; i < 8; i++)
        {
            auto ldA = __cvta_generic_to_shared(&sharedA[((warpId / 2) * 64 + i * 8 + laneId % 8) * sharedLda + (BLOCK_M + offset) * ((laneId / 4) & 1)]);
            auto ldB = __cvta_generic_to_shared(&sharedB[((warpId & 1) * 64 + i * 8 + laneId % 8) * sharedLdb + (BLOCK_M + offset) * ((laneId / 4) & 1)]);
            asm volatile(
