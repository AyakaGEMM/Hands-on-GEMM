...
#pragma unroll
        ...
        {
            auto ldA = __cvta_generic_to_shared(&sharedA[((warpId / 2) * 64 + laneId + ik * BLOCK_M) * sharedLda]);
            auto ldB = __cvta_generic_to_shared(&sharedB[((warpId % 2) * 64 + laneId + ik * BLOCK_N) * sharedLdb]);
