...
#pragma unroll
        for (int i = 0; i < 8; i++)
        {
            frag_a[i] = *reinterpret_cast<int32_t *>(&sharedA[((warpId / 2) * 64 + i * 8 + laneId / 4) * sharedLda + (laneId % 4) * 4]);
            frag_b[i] = *reinterpret_cast<int32_t *>(&sharedB[((warpId & 1) * 64 + i * 8 + laneId / 4) * sharedLdb + (laneId % 4) * 4]);
        }
#pragma unroll
        for (int i = 0; i < 8; i++)
        {
            frag_a[i + 8] = *reinterpret_cast<int32_t *>(&sharedA[((warpId / 2) * 64 + i * 8 + laneId / 4 + BLOCK_M + offset) * sharedLda + (laneId % 4) * 4]);
            frag_b[i + 8] = *reinterpret_cast<int32_t *>(&sharedB[((warpId & 1) * 64 + i * 8 + laneId / 4 + BLOCK_N + offset) * sharedLdb + (laneId % 4) * 4]);
        }
