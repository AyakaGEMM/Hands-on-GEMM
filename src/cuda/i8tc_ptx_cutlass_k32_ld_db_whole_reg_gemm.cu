...
#pragma unroll
    for (int i = 0; i < 8; i++)
    {
        auto ldA = __cvta_generic_to_shared(&sharedA[((warpId / 2) * 64 + i * 8 + laneId % 8 + (BLOCK_M + offset) * ((laneId / 8) & 1)) * sharedLda + stage * sharedASize]);
        auto ldB = __cvta_generic_to_shared(&sharedB[((warpId & 1) * 64 + i * 8 + laneId % 8 + (BLOCK_N + offset) * ((laneId / 8) & 1)) * sharedLdb + stage * sharedBSize]);
        asm volatile(
