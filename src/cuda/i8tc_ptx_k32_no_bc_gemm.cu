                              ...
                              int M, int N, int K, const int32_t alpha, const int32_t beta)
{
    ...
    __shared__ __align__(16 * 1024) int8_t shared_mem[260 * sharedLda + 260 * sharedLdb];
    auto sharedA = shared_mem;
    auto sharedB = shared_mem + 260 * sharedLda;
...
#pragma unroll
    ...
    {
        // Do 32x32x32 (mnk) mma at a time to avoid excessive sectors.
        *reinterpret_cast<int32_t *>(&sharedA[(baseIdx / 8 + ((baseIdx / 4) % 2) * 132) * sharedLda + ((baseIdx & 3) << 2)]) = *reinterpret_cast<const int32_t *>(&baseA[(baseIdx / 8) * lda + ((baseIdx & 7) << 2) + k]);
        *reinterpret_cast<int32_t *>(&sharedA[(baseIdx / 8 + 64 + ((baseIdx / 4) % 2) * 132) * sharedLda + ((baseIdx & 3) << 2)]) = *reinterpret_cast<const int32_t *>(&baseA[(baseIdx / 8 + 64) * lda + ((baseIdx & 7) << 2) + k]);

        // Need transpose here, I leave it here for now.
        *reinterpret_cast<int32_t *>(&sharedB[(baseIdx / 8 + ((baseIdx / 4) % 2) * 132) * sharedLdb + ((baseIdx & 3) << 2)]) = *reinterpret_cast<const int32_t *>(&baseB[(baseIdx / 8) * ldb + ((baseIdx & 7) << 2) + k]);
        *reinterpret_cast<int32_t *>(&sharedB[(baseIdx / 8 + 64 + ((baseIdx / 4) % 2) * 132) * sharedLdb + ((baseIdx & 3) << 2)]) = *reinterpret_cast<const int32_t *>(&baseB[(baseIdx / 8 + 64) * ldb + ((baseIdx & 7) << 2) + k]);
...
#pragma unroll
        for (int i = 0; i < 4; i++) // 8 byte load.
        {
            frag_a[i * 2 + 1] = *reinterpret_cast<const int32_t *>(&sharedA[(laneId % 4) * 4 + ((warpId / 4) * 32 + laneId / 4 + i * 8 + 132) * sharedLda]);
            frag_b[i * 2 + 1] = *reinterpret_cast<const int32_t *>(&sharedB[(laneId % 4) * 4 + ((warpId & 3) * 32 + laneId / 4 + i * 8 + 132) * sharedLdb]);
        }
        // Do mma.
#pragma unroll
        for (int i = 0; i < 16; i++)
        {
            asm(
                ...
                : "r"(frag_a[(i >> 2) << 1]), "r"(frag_b[(i & 3) << 1]), "r"(frag_a[((i >> 2) << 1) + 1])); // With an implicit __syncwarp() here.
        }
...
#pragma unroll
        for (int i = 0; i < 16; i++)
        {
            asm(
                ...
                {%0, %1};"
                : "+r"(frag_c[i][0]), "+r"(frag_c[i][1])
                : "r"(frag_a[((i >> 2) << 1) + 1]), "r"(frag_b[((i & 3) << 1) + 1])); // With an implicit __syncwarp() here.
        }
